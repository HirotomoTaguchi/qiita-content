...
tags:
  ...
  - MicrosoftSecurity
private: false
updated_at: '2026-01-05T21:18:01+09:00'
...
Microsoft Defender for Endpoint(MDE) をはじめとするEDRは、システムの複数の監視ポイントでふるまいを分析しますが、ログの「保存」においては「重要そうなもの」だけをフィルタリングして保存する仕組みになっていることが多いです。そうでもしないと、ログ量がとんでもないことになります。これは帯域やストレージコストの観点では合理的ですが、組織固有の監視要件には対応しきれない場合があります。そんな中、MDEではカスタムデータ収集機能という、これまで取得していなかったログを取得する機能がプレビューで出てきたので、簡単にまとめておきます。[^1]
...
簡単に言えば、「MDEが通常は捨ててしまうイベント指定して記録する」機能です。通常、カーネルドライバやAMSIなど、システムの様々なレイヤーで大量のイベントを観測し、分析していますが、すべてをクラウドに保存するわけにはいきません。そこでMicrosoft側が「これは重要そう」と判断したイベントだけを選別・整形して保存しています。これはどのEDRでも仕組みは大体同じです。
...
- 1ルール及び1デバイスあたり24時間で最大25,000イベントまでしか収集されません。フィルタが甘いとすぐ上限に達して収集が止まってしまいそうです。
...
そこで、DeviceCustomScriptEvents テーブルを利用することで、特定のインタプリタやスクリプトファイルに対する監査証跡を明示的に保存できます。これは内部的に AMSI (Antimalware Scan Interface) のような機構を通じてデコードされたスクリプト内容を取得していると推測されますが、後述するように実行されたスクリプトを確認できるという強力な利点があります。
...
:::alert
簡易的なデモ目的であり、実際の攻撃はより複雑な難読化を使用します。
:::

標準の MDE ログ（`DeviceProcessEvents`）では、プロセスが起動された事実は記録されますが、その引数（コマンドライン）は攻撃者が隠ぺいした「意味不明な英数字の羅列」のまま記録されます。Defenderはこれらをバックエンドでデコードされたものを分析し[^2]、危険だと判定したらアラートとして証拠を残しますが、Defenderのアラートの網にかからなかったものは捨てられてしまい、難読化されたものだけが残ります。

:::alert
何度も言うようですが、バックエンドではデコードされてMDEで分析されています。あくまで、Advanced Huntingのログとして残るかという点が論点です。
:::
...
## Azure Monitor Agent（AMA）とのすみわけ

- TBU

## 他の活用シーン

今回は PowerShell の難読化解除に焦点を当てましたが、`DeviceCustomScriptEvents` は他にも多様なシナリオで威力を発揮しそうです。とはいえ、コストを含めた有効なシナリオの検証が追いついていない部分があるので、随時アップデートするか他のブログにします。
